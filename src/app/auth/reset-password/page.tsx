...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!accessToken) { setError("Invalid reset link. Please request a new one."); return; }
    setError("");
    setLoading(true);
    try {
        ...
        {success ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              <div>
                <label className="block text-[13px] font-medium text-[var(--text-primary)] mb-1.5">New password</label>
                  ...
                  <button type="button" tabIndex={-1} onClick={() => setShow(s => !s)}
                    ...
                    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.7" className="w-4 h-4">
                      {show
                        ...
                        : <><path d="M10 4.5C5.5 4.5 1.5 10 1.5 10S5.5 15.5 10 15.5 18.5 10 18.5 10 14.5 4.5 10 4.5z" /><circle cx="10" cy="10" r="2.5" /></>
                      }
                    </svg>
                  </button>
                </div>
              ...
              <div>
                <label className="block text-[13px] font-medium text-[var(--text-primary)] mb-1.5">Confirm password</label>
                <input
                  type={show ? "text" : "password"}
                  value={confirm}
